...
<div class="row">
    <div class="col-12">
            ...
            <a href="{{route('facture_create')}}" class="btn btn-primary mb-2">Créer un nouveau</a>
            @endcan
      <div class="card">
            ...
            <tbody>
                @foreach ($factures as $facture)
                    <tr>
                        ...
                        @if (Auth::user()->is_admin)<td> {{$facture->client->nom}} </td> @endif
                        <td>{{$facture->societe->libelle}}</td>
                        <td>{{date('d/m/Y',strtotime($facture->date))}}</td>
                        <td class="d-flex align-items-center">
                          <a href="{{route('facture_show',$facture->id)}}" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a>
                          @can('update', $facture)
